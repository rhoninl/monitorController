...
def _get_physical_monitors_from_hmonitor(hmonitor: wintypes.HMONITOR) -> list:
    ...
    # create array
    phy_monitor_array = (_PhysicalMonitorStructure *
                         phy_monitor_number.value)()
...
class PhyMonitor(object):
    ...
    def _get_model_info(self):
        ...
        if info_display_type == '':
            _LOGGER.warning('unable to find display type info in vcp caps string: {}'.format(
                self._caps_string))
    ...
    def color_temperature(self):
        increment = self.get_vcp_value_by_name(
            'User Color Temperature Increment')[0]
    ...
    def color_preset(self, preset: str):
        ...
        self.set_vcp_value_by_name(
            'Select Color Preset', vcp_code.COLOR_PRESET_CODE.get(preset))
    ...
    def auto_setup_perform(self):
        ...
        """
        self.set_vcp_value_by_name(
            'Auto Setup', vcp_code.AUTO_SETUP_CODE.get('Manual Perform'))
    ...
    def osd_language(self, language: str):
        if language not in self.osd_languages_list:
            ...
            return
        self.set_vcp_value_by_name(
            'OSD Language', vcp_code.OSD_LANG_CODE.get(language))
    ...
    def power_mode(self, mode: str):
        if mode not in self.power_mode_list:
            ...
            return
        self.set_vcp_value_by_name(
            'Power Mode', vcp_code.POWER_MODE_CODE.get(mode))
    ...
    def input_src(self, src: str):
        if src not in self.input_src_list:
            ...
            return
        self.set_vcp_value_by_name(
            'Input Source', vcp_code.INPUT_SRC_CODE.get(src))

    @property
    def info_pannel_type(self) -> str:
        pannel_type = self.get_vcp_value_by_name(
            'Flat Panel Sub-Pixel Layout')[0]
